...
beforeEach(async () => {
  ...
  const memoryPath = join(harness.home, "kb", "research", "memories", "recoverable-memory.md");
...
test("retry completes a reflect committed to history without duplicating it", async () => {
  ...
  const interrupted = await harness.run("kb", ["reflect", "--complete", "--in", "research"], {
  ...
  const recovered = await harness.run("kb", ["reflect", "--complete", "--in", "research"], {
  ...
  expect(recovered.stdout).toContain("Completed Reflect handoff: 1 Memory\n");
...
test("retry before acknowledgement uses the durable receipt instead of a new wall-clock instant", async () => {
  ...
  const retryInstant = "2026-07-08T12:00:00.000Z";
  expect((await harness.run("kb", ["reflect", "--in", "research"], { env: { KB_NOW: instant } })).code).toBe(0);
  const failedBeforeAcknowledgement = await harness.run("kb", ["reflect", "--complete", "--in", "research"], {
...
test("failure before the history commit retains the prior event and retry creates one event", async () => {
  const instant = "2026-07-07T12:00:00.000Z";
  expect((await harness.run("kb", ["reflect", "--in", "research"], { env: { KB_NOW: instant } })).code).toBe(0);
  const failed = await harness.run("kb", ["reflect", "--complete", "--in", "research"], {
  ...
  expect((await harness.run("kb", ["reflect", "--complete", "--in", "research"], { env: { KB_NOW: instant } })).code).toBe(0);
  expect((await readFile(join(kbPath, "log.md"), "utf8")).match(/^## .* reflect \|/gm)).toHaveLength(1);
});
...
test("concurrent reflects serialize into coherent events", async () => {
  ...
  expect(history.match(/^## .* reflect \|/gm) ?? []).toHaveLength(0);
  expect((await harness.runKb(["reflect", "--complete", "--in", "research"])).code).toBe(0);
  const completedHistory = await readFile(join(kbPath, "log.md"), "utf8");
  expect(completedHistory.match(/^## .* reflect \|/gm)).toHaveLength(1);
  expect(await readFile(join(kbPath, "kb.yaml"), "utf8")).toContain(`lastReflectAt: ${instant}`);
});
...
test("a process death before acknowledgement is recovered from the durable receipt", async () => {
  ...
  const retryInstant = "2026-07-08T12:00:00.000Z";
  expect((await harness.run("kb", ["reflect", "--in", "research"], { env: { KB_NOW: instant } })).code).toBe(0);
  const exited = await harness.run("kb", ["reflect", "--complete", "--in", "research"], {
...
test("every transition boundary recovers after ordinary failure and process death", async () => {
  ...
  for (const injection of ["KB_FAIL_REFLECT_TRANSITION", "KB_EXIT_REFLECT_TRANSITION"] as const) {
    for (const phase of phases) {
      ...
      const memoryPath = join(kbPath, "memories", "boundary-memory.md");
      await utimes(memoryPath, new Date("2026-07-01T00:00:00.000Z"), new Date("2026-07-01T00:00:00.000Z"));
      expect((await harness.run("kb", ["reflect", "--in", name], { env: { KB_NOW: instant } })).code).toBe(0);
      const interrupted = await harness.run("kb", ["reflect", "--complete", "--in", name], {
      ...
      const recovered = await harness.run("kb", ["reflect", "--complete", "--in", name], { env: { KB_NOW: instant } });
      expect(recovered.code).toBe(0);
      expect(
        (await readFile(join(kbPath, "log.md"), "utf8")).match(/^## .* reflect \|/gm),
        `${injection}:${phase}`,
      ).toHaveLength(1);
      expect(await readFile(join(kbPath, "kb.yaml"), "utf8")).toContain(`lastReflectAt: ${instant}`);
